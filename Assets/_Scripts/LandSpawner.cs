...
public class LandSpawner : MonoBehaviour
{
    ...
    [SerializeField]
    private Tuple<string, Sprite>[] landNameSprites;
    ...
    IEnumerator SpawnLandIngredient()
    {
        while (true)
        {
            ...
            if (chance == 1)
            {
                ...
                int randInd = UnityEngine.Random.Range(0, landNameSprites.Length);
                ingredient.GetComponent<LandIngredient>().SetIngredientInfo(landNameSprites[randInd].Item1, landNameSprites[randInd].Item2);
